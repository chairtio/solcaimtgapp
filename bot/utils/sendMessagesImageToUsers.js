...
const message = `Join the [*Live audio space*](https://x.com/solclaimx/status/1827703417134829680) on X with [*ElementalBlast*](http://t.me/elementalblastnews)!

🤝 Together with the first project that will launch 7 games on Telegram.

👀 Site reveal will be in the audio call.

⚠️ Not only that, we will present a new exciting feature coming soon!

💰 Be sure [*to tune in*](https://x.com/solclaimx/status/1827703417134829680) and discover when we launch the first real $SOL utility token.

Giveaways in the call for the community!`;
...
export const sendMessagesImageToUsers = async (bot) => {
    try {
        ...
        for (let start = 0; start < userIds.length; start += BATCH_SIZE) {
            ...
            for (let i = 0; i < batch.length; i++) {
                ...
                try {
                    await bot.telegram.sendPhoto(userId, 'AgACAgUAAyEFAASCbRtdAAIEHGbMPf_JHjsJmKDZg9FCYUPSU0zZAAKbwTEbdAFhVvu44XzxwjzIAQADAgADeAADNQQ', {
                        caption: escapedMessage,
                        parse_mode: 'MarkdownV2',
                        reply_markup: {
                            inline_keyboard: [
                                [{ text: '🚨 Set Reminder on X', url: 'https://x.com/solclaimx/status/1827703417134829680' }]
                            ],
                        },
