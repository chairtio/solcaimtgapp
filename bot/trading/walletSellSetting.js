...
export const walletSellSetting = async (ctx, walletId, mintAddressStr) => {
  try {
    ...
    if (wallet) {
      ...
      // Optionally, refresh the Sell Mode message to reflect changes
      await sellMode(ctx, { mintAddressStr, editMessageTradeId: ctx.callbackQuery.message.message_id });
    } else {
...
export const handleSelectAllSell = async (ctx, mintAddressStr) => {
  ...
  try {
    ...
    // Refresh the sell Mode message to reflect changes
    await sellMode(ctx, { mintAddressStr, editMessageTradeId: ctx.callbackQuery.message.message_id });
  } catch (error) {
    console.error('Error handling Select All action wallet sell setting:', error.message);
    await ctx.reply('An error occurred while processing your request.');
  }
};

export const handleUnselectAllSell = async (ctx, mintAddressStr) => {
  const userId = ctx.from.id;
  ...
  try {
      ...
      console.error('Error handling Unselect All action wallet sell setting:', error.message);
      await ctx.reply('An error occurred while processing your request.');
  }
};

export const handleSelectAllWalletSell = async (ctx) => {
  try {
    ...
    switch (action) {
      case 'selectWalletsS':
        await handleSelectAllSell(ctx, mintAddressStr);
        break;
      case 'unselectWalletsS':
        await handleUnselectAllSell(ctx, mintAddressStr);
        break;
    ...
    console.error('Error handling handleSelectAllWallet SELL setting:', error.message);
    await ctx.reply('An error occurred while processing your request.');
  }
};
