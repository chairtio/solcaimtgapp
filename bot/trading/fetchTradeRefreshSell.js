...
import { isValidMintAddress, getMintInfo, escapeMarkdownV2, formatMarketCap, formatTVL, getTokenBalances, formatAmountToken } from './utilsTrading/tokenUtils.js';
...
export async function fetchTradeRefreshSell(ctx) {
  try {
    let mintAddressStr;
    ...
    if (ctx.message && ctx.message.text) {
      ...
      if (input.length < 2) {
        await ctx.reply('Usage: /fetchdata <mintAddress>. Please provide a mint address to fetch data.');
        return;
      }
      mintAddressStr = input[1];
    } else if (ctx.callbackQuery && ctx.callbackQuery.data) {
      const callbackData = ctx.callbackQuery.data.split(':');
      if (callbackData[0] === 'refreshSell' && callbackData.length > 1) {
        mintAddressStr = callbackData[1];
      } else {
        await ctx.reply('Refresh button data is invalid.');
        return;
      }
    } else {
    ...
    if (!error.message.includes('specified new message content and reply markup are exactly the same as a current content and reply markup of the message')) {
      console.error('Error fetching data fetch Trade Refresh sell:', error.message);
      await ctx.reply('No liquidity found yet. Please try again later.');
    }
  }
}
