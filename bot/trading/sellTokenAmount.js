...
export async function sellTokenAmount(ctx, options = {}) {
  try {
    ...
    const amountToken = parseFloat(ctx.message.text.trim());
    const mintAddress = options.mintAddressStr || (ctx.callbackQuery?.data.split(':')[1] || '');

    if (!mintAddress || isNaN(amountToken) || amountToken <= 0) {
      const originalMessageId = ctx.message.reply_to_message.message_id;
      await Promise.all([
        pTimeout(ctx.reply('❌ Invalid amount or missing mint address. Please enter a valid number.'), 10000),
        deleteMessages(ctx, [ctx.message.message_id, originalMessageId])
      ]);
      return;
    }
    ...
    const originalMessageId = ctx.message.reply_to_message.message_id;
    const deleteMessagePromise = deleteMessages(ctx, [ctx.message.message_id, originalMessageId]);
    ...
    const transactions = Object.entries(userWallets).map(async ([walletId, privateKey]) => {
      try {
        ...
        if (tokenBalance < parseFloat(amountToken)) {
          await ctx.telegram.editMessageText(ctx.chat.id, progressMessages[walletId], undefined, `🔴 Insufficient token for sell \\| Wallet: \`${escapeMarkdownV2(userpublicKey)}\``, {
            parse_mode: 'MarkdownV2'
          });
          return;
        }

        const amountTokenSell = parseFloat(amountToken.toFixed(decimals));
