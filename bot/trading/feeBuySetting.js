...
export async function feeBuySetting(ctx, { editMessageTradeId, mintAddressStr, amount = null } = {}) {
    try {
        ...
        await buyMode(ctx, { editMessageTradeId, mintAddressStr, currentMessageId, originalMessageId });

    } catch (error) {
        console.error('Error setting fee buy setting:', error.message);
